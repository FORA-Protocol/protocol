...
def main(src_dir, desc_path, out_file, required_path=None, unique_path=None):
    ...
    # the package literals above are the only place this script knows the wire's name,
    # so an empty match means they have drifted from the proto, not that there is
    # nothing to do.
    if not matched:
        raise SystemExit(
