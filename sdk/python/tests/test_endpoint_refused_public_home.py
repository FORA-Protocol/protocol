...
package root; this one was not, so ``fora_sdk.EndpointRefusedError`` did not exist
while ``fora_sdk.resolvers.EndpointRefusedError`` did.
...
import fora_sdk
from fora_sdk.resolvers import EndpointRefusedError


def test_endpoint_refused_error_is_exported_from_the_package_root() -> None:
    assert "EndpointRefusedError" in fora_sdk.__all__
    assert fora_sdk.EndpointRefusedError is EndpointRefusedError
...
def test_it_sits_beside_its_siblings() -> None:
    ...
    for sibling in (
        ...
        assert sibling in fora_sdk.__all__
