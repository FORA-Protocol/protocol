...
An Exchange MAY publish ``AccountRegistration.data_schema`` in its ``fora.json``: a
...
from fora_sdk._jsondepth import _raw_nesting_depth
...
# member AS SERVED in fora.json — which is why the compile face takes raw bytes
...
def is_safe_schema_pattern(pattern: str) -> bool:
    ...
    and rewriting ``$``. See ``_fora_pattern`` below.
...
_ForaValidator = jsonschema.Draft202012Validator
...
def _decode(raw: bytes) -> tuple[Any, SchemaVerdict]:
    ...
    # the fora.json member this schema lives in.
...
def compile_registration_schema(raw: bytes) -> tuple[RegistrationSchema | None, SchemaVerdict]:
    ...
    in fora.json — because MAX_REGISTRATION_SCHEMA_BYTES is defined over those bytes.
