...
FORA's failure envelope is a typed ``ErrorDetail`` attached to the transport
...
ERROR_DETAIL_TYPE = "fora.v1.ErrorDetail"
...
def _proto_names(payload: Any) -> Any:
    ...
    Connect's error-detail ``debug`` projection is the one place a FORA payload arrives
    ...
    the FORA wire — so without this the reason block is silently dropped as an unknown
...
def error_detail_from(err: Mapping[str, Any] | Iterable[Any]) -> ErrorDetail | None:
    """Extract the first FORA ErrorDetail from a Connect error (or its details).
