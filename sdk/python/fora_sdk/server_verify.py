...
Signature-Input/Signature off the request headers, (b) enforces the FORA
...
# The FORA required covered set, mirroring Go helpers.requiredCoveredComponents.
...
def _parse_signature_input(signature_input: str) -> _ParsedInput | None:
    ...
    FORA covered set, string-valued keyid). It extracts the label, the covered
...
def _quoted_tokens(inner: str) -> list[str]:
    ...
    Only ``"name"`` tokens are collected (the FORA covered set carries no
...
def verify_request_server(
    *,
    ...
    # the five FORA components must be declared, or a signer omitting a bound field
...
def verify_multisig_request_server(
    *,
    ...
    """Verify an inbound MULTISIG forwarding-chain FORA request; return a
