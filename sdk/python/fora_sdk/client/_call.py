...
Every FORA RPC is unary, so a Connect-unary JSON client is the whole protocol rather than
a subset of it: ``POST /fora.v1.<Service>/<Method>`` with a JSON body. Full Connect
framing exists for streaming, which FORA has none of and would never use, and it would
...
#: Caps the response body a single FORA call will read.
#:
...
#: Bounds one call, in seconds. A FORA RPC is interactive — something is waiting on the
