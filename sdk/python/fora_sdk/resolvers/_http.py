...
The SDK's trust core (``fora_sdk.core`` / the signing primitives) IMPORTS no
...
:mod:`fora_sdk.resolvers._ssrf`), and connects PINNED to a checked IP — closing
...
def ssrf_guard() -> httpx.HTTPTransport:
    ...
    target raises :class:`~fora_sdk.resolvers._ssrf.SsrfError` (an ``OSError``).
    """
...
def async_ssrf_guard() -> httpx.AsyncHTTPTransport:
    ...
    :class:`~fora_sdk.resolvers._ssrf.SsrfError` (an ``OSError``).
    """
