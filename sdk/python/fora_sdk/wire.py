...
#: ``/.well-known/fora.json`` document carries its own version in a separate
#: namespace, which this constant does NOT supply — that is
#: :data:`WellKnownManifestVersion`.
ProtocolVersion = "1.0"
#: The version of the ``/.well-known/fora.json`` DOCUMENT layout, stamped on
...
#: ``{scheme}://{host}{WellKnownPath}`` to learn the endpoint and the keys.
#: ``WellKnownManifestVersion`` versions the document's CONTENT; this names
#: WHERE it is served.
...
def manifest_version_refusal(ver: object) -> str | None:
    ...
    :class:`fora_sdk.resolvers.errors.ManifestVersionRefusedError`.
    """
