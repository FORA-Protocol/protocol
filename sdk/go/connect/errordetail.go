...
// ErrorDetailFrom extracts the first FORA ErrorDetail attached to err's Connect
...
// errorDetailFromConnect reads the first FORA ErrorDetail off an already-unwrapped
// Connect error. Split out so the CallError bridge can reuse the one extraction
// rather than restating it.
func errorDetailFromConnect(cerr *connectrpc.Error) (*forav1.ErrorDetail, bool) {
	for _, d := range cerr.Details() {
		msg, verr := d.Value()
		if verr != nil {
			continue
		}
		if ed, ok := msg.(*forav1.ErrorDetail); ok {
			return ed, true
		}
	}
	return nil, false
