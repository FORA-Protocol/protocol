...
type recordingAccount struct {
	...
	gotRegister *forav1.RegisterRequest
	gotStatus   *forav1.GetAccountStatusRequest
	registers   int
...
func (a *recordingAccount) Register(
	_ context.Context, req *connectrpc.Request[forav1.RegisterRequest],
) (*connectrpc.Response[forav1.RegisterResponse], error) {
	a.gotRegister = req.Msg
	a.registers++
	if a.refuse != nil {
		return nil, a.refuse
	}
	return connectrpc.NewResponse(&forav1.RegisterResponse{
		Ver: helpers.ProtocolVersion, BillingRef: a.billingRef, Active: a.active,
	}), nil
}

func (a *recordingAccount) GetAccountStatus(
	_ context.Context, req *connectrpc.Request[forav1.GetAccountStatusRequest],
...
) (*foraconnect.CallError, *recordingAccount) {
	t.Helper()
	sig := newSigningFixture(t)
	origin := &recordingAccount{billingRef: "acct-1"}
	domain, _ := selfAdvertisingExchangeWith(t, sig, origin, func() map[string]any {
...
func TestRegister_PreCheckRefusalCarriesItsFieldErrors(t *testing.T) {
	...
	detail, ok := foraconnect.ErrorDetailFrom(cerr)
	if !ok {
...
func TestRegister_ClassifiesARefusedRequirementsRead(t *testing.T) {
	for _, tc := range []struct {
		name string
		err  error
		want foraconnect.CallErrorKind
	}{
		{"not a host", fmt.Errorf("%w: %q is not a bare domain",
			helpers.ErrInvalidHost, "exchange.test/path"), foraconnect.CallNotSent},
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			_, err := client.Register(context.Background(),
				&forav1.RegisterRequest{Exchange: domain})
...
func TestRegister_ARepeatReturnsTheSameHandle(t *testing.T) {
	...
	second, err := client.Register(context.Background(), &forav1.RegisterRequest{Exchange: domain})
	if err != nil {
...
func TestRegister_TypedRefusalIsReadableThroughErrorDetailFrom(t *testing.T) {
	...
	detail := helpers.RegistrationFailureDetail(
		"fora.v1.ExchangeService", "terms have moved",
...
func TestAccountVerbs_RefuseAnUnaddressedRequest(t *testing.T) {
	...
	for _, exchange := range []string{"", "https://exchange.test", "exchange.test/path", "ex_change.test"} {
		...
		t.Run(name, func(t *testing.T) {
			_, err := client.Register(context.Background(),
				&forav1.RegisterRequest{Exchange: exchange})
			assertNotSent(t, err)
...
func TestAccountVerbs_CallerVerWins(t *testing.T) {
	...
	if _, err := client.GetAccountStatus(context.Background(),
		&forav1.GetAccountStatusRequest{Ver: "9.9", Exchange: domain}); err != nil {
		t.Fatalf("get account status: %v", err)
	}
...
func TestGetAccountStatus_IdenticalRequestsCollideUnlessTheWindowMoves(t *testing.T) {
	...
	serve := func(t *testing.T, sig signingFixture, svc forav1connect.ExchangeServiceHandler) string {
		t.Helper()
		path, h := foraserver.NewExchangeServiceHandler(svc,
			foraserver.WithKeyResolver(sig.resolver),
			foraserver.WithReplayStore(newMemReplayStore()))
