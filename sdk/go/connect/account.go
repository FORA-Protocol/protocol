...
const clientErrorDomain = "fora.v1.Client"
...
func (c *Client) applyRegistrationRequirements(
	ctx context.Context, op string, sent *forav1.RegisterRequest,
) error {
...
func renderFieldErrors(fails []*forav1.RegistrationFieldError) string {
	out := ""
	for i, f := range fails {
		if i > 0 {
			out += "; "
