...
const testServiceDomain = "fora.v1.ExchangeService"
...
func TestAttachErrorDetail_StampsEnvelopeOnClassifiedError(t *testing.T) {
	...
	out := foraserver.AttachErrorDetail(cerr, testServiceDomain, "bad idempotency_key", meta)
	...
	ed, ok := foraconnect.ErrorDetailFrom(out)
	if !ok {
...
func TestAttachErrorDetail_EmptyMetadataIsAbsent(t *testing.T) {
	...
	out := foraserver.AttachErrorDetail(cerr, testServiceDomain, "boom", nil)
...
func TestNewErrorDetail_BuildHalfMatchesAttachAndStaysMutable(t *testing.T) {
	...
	if d := foraserver.NewErrorDetail(testServiceDomain, "boom", nil); len(d.GetMetadata()) != 0 {
		t.Fatalf("Metadata = %v, want absent for nil input", d.GetMetadata())
	}
	meta := map[string]string{"field": "quantity"}
	d := foraserver.NewErrorDetail(testServiceDomain, "bad quantity", meta)
	...
	d.Reason = &forav1.ErrorDetail_TransactionDenial{
		TransactionDenial: &forav1.TransactionDenial{Reason: forav1.DenialReason_DENIAL_REASON_OFFER_EXPIRED},
	}
	cerr := connectrpc.NewError(connectrpc.CodeFailedPrecondition, errors.New("bad quantity"))
	ed, ok := foraconnect.ErrorDetailFrom(foraserver.AttachDetail(cerr, d))
	if !ok {
		t.Fatal("ErrorDetailFrom returned false")
	}
	if reason, _ := helpers.Reason(ed).(forav1.DenialReason); reason != forav1.DenialReason_DENIAL_REASON_OFFER_EXPIRED {
...
func TestAttachDetail_RoundTripsTypedReason(t *testing.T) {
	...
	out := foraserver.AttachDetail(cerr, d)

	ed, ok := foraconnect.ErrorDetailFrom(out)
	if !ok {
		t.Fatal("ErrorDetailFrom returned false")
	}
	reason, ok := helpers.Reason(ed).(forav1.DenialReason)
	if !ok {
		t.Fatalf("Reason type = %T, want forav1.DenialReason", helpers.Reason(ed))
	}
	if reason != forav1.DenialReason_DENIAL_REASON_INSUFFICIENT_BALANCE {
