...
not come from a FORA service — a draining load balancer, a proxy's own page — carries no
...
## One bootstrap coordinate, named once per language

The well-known document's path is the protocol's single bootstrap coordinate: a client
holding nothing but a hostname fetches `{scheme}://{host}{WellKnownPath}` to learn the
endpoint and the keys, and everything else in a session is reached from what that
document says. So the three SDKs agreeing on that one string is the precondition for
interop, not a tidiness concern.

It was nonetheless built inline at each call site, once per language, which is the shape
that lets one port drift without anything failing loudly. A wrong path is not a transport
error: it fetches *some other document* from the right host, and the read falls through to
a role check written to catch a different mistake. Naming the value once per language and
pinning the three against each other in the wire-constants corpus is what turns that drift
into a test failure instead of a silent misfetch.

The constant already existed when the registration-requirements reader landed, but only
two of the three endpoint resolvers read it, and the new reader is a *third* leg building
the same URL. Leaving it inline would have added a site per language while the remedy was
already exported. All four URL-building sites read the constant now — the new reader in
Go, Python and TypeScript, and the TypeScript endpoint resolver that had been left behind.
Doc comments still spell the path out; prose describing a value is not a second definition
of it.
