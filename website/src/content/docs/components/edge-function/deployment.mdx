---
title: "Deployment"
description: "Deployment models, environment configuration, secrets management, and provider onboarding for the FORA Edge Function"
---
...
sequenceDiagram
    ...
    participant FORA as FORA CLI

    Provider->>MO: Sign resource access agreement
    MO-->>Provider: Exchange endpoint + credentials

    Provider->>FORA: fora init --cdn cloudfront --exchange exchange.ssp.com
    Note over FORA: Generates edge function config<br/>Outputs CDN-specific deployment artifact

    FORA-->>Provider: EdgeConfigDocument + deployment instructions
    ...
    Provider->>CDN: Verify: GET /.well-known/fora.json
...
3. **Verify** -- `curl -I https://techcrunch.com/.well-known/fora.json` returns the manifest.
...
The reference implementation is a vanilla JavaScript CloudFront Function that handles bot detection, signed URL verification, and `fora.json` serving.
...
|---|---|---|---|
| Exchange endpoint URL | Provider-Exchange contract | Rarely (months) | `https://exchange.ssp.com/fora/v1` |
...
interface EdgeConfigDocument {
  ...
  /** Authorized Exchanges (becomes fora.json exchanges[]). */
  exchanges: Array<{
    domain: string;
    endpoint: string;
...
During provider onboarding, the FORA CLI writes ACME challenge tokens to the CDN's KV store. The edge function auto-serves these tokens at `/.well-known/fora-verify/{token}`, eliminating the need for providers to manually place files on their origin server.

```
fora-cli writes to KV:
  Key:   fora-verify:{token}
  Value: {token}
  TTL:   10 minutes

Edge function route:
  /.well-known/fora-verify/* -> read from KV -> return as text/plain
```
...
For providers without the FORA edge function deployed, `fora-cli` provides fallback instructions to manually place the token file on their web server.

## Configurable Serving Modes for fora.json and rsl.txt
...
```
GET https://exchange.ssp-example.com/provider/techcrunch.com/fora.json
...
FORA_PROVIDER=techcrunch.com
FORA_EXCHANGE_DOMAIN=exchange.ssp-example.com
FORA_EXCHANGE_ENDPOINT=https://exchange.ssp-example.com/fora/v1
FORA_EXCHANGE_RELATIONSHIP=DIRECT
FORA_CONTACT=licensing@techcrunch.com
```

The edge function constructs fora.json and rsl.txt at startup. Simplest approach, suitable for providers with a single Exchange relationship that changes infrequently.

**Mode: kv**

The full fora.json and rsl.txt content is stored in the CDN's edge KV (Workers KV, CloudFront KeyValueStore, EdgeKV, Fastly Config Store). A separate management process writes to KV when configuration changes.
...
sequenceDiagram
    ...
    Agent->>Edge: GET /.well-known/fora.json (or /rsl.txt)

    alt Cache hit (KV)
        Edge->>Cache: lookup("fora.json")
        Cache-->>Edge: cached content
        Edge-->>Agent: 200 OK (from cache)
    else Cache miss
        Edge->>MP: GET /provider/{domain}/fora.json
        MP-->>Edge: fora.json content
        Edge->>Cache: store("fora.json", content, TTL=3600s)
...
| Applies to | Both fora.json and rsl.txt | Same caching strategy for both files |
...
1. `GET /.well-known/fora.json` returns valid manifest with `200 OK`.
