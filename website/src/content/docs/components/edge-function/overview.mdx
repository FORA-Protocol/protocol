...
description: "CDN-level enforcement point for AI resource access in the FORA protocol"
---
...
3. **Serve fora.json and rsl.txt** -- generate, cache, and serve `/.well-known/fora.json` and `/rsl.txt` so protocol-aware agents discover the Exchange without triggering a 403.
...
5. **Serve ACME domain verification challenges (v1.0)** -- auto-serve challenge tokens at `/.well-known/fora-verify/{token}` from the CDN's KV store during provider onboarding domain verification.
...
2. **Deploy the edge function** using the [Terraform module](https://github.com/FORA-Protocol/protocol/tree/main/terraform/aws-fora-edge/).
3. **Verify** -- `curl -I https://example.com/.well-known/fora.json` returns the manifest.
...
```mermaid
flowchart TD
    REQ[Incoming Request] --> FORA_JSON{Path = /.well-known/fora.json?}
    FORA_JSON -->|Yes| SERVE_MANIFEST[Serve WellKnownManifest<br/>200 OK + Cache-Control]
    FORA_JSON -->|No| RSL_TXT{Path = /rsl.txt?}
...
```
handleRequest(ctx):
  1. If path == "/.well-known/fora.json":
       ...
       -> For self-attesting providers, the verifier keys live in the WBA
          ...
          claims schema in ext["fora.attestation.claims_schema"]
       -> Return 200
  ...
  1d. If path starts with "/.well-known/fora-verify/":
