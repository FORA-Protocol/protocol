...
Omgilibot               - Webz.io
FacebookBot             - Meta
fora-ai-buyer          - FORA protocol agent (self-identified)
```
...
|---|---|---|
| **Edge function** (FORA) | Cooperative AI bots (ClaudeBot, GPTBot, etc.) | User-Agent string matching |
...
**Integration requirement**: The edge function runs *after* the WAF (or alongside it, depending on CDN architecture). WAF rules should not block requests to `/.well-known/fora.json` or `/rsl.txt`, and should not interfere with signed URL parameters. Providers must allowlist these paths in their WAF configuration.
...
X-Content-Rules: https://exchange.ssp-example.com/fora/v1
...
```json
{
  ...
  "protocol": "FORA",
  "version": "1.0",
  "info_url": "https://exchange.ssp-example.com/fora/v1/info",
  "fora_json_url": "https://techcrunch.com/.well-known/fora.json"
}
```

**Header semantics**:
- `X-Content-Rules` -- the Exchange info endpoint. This is the key discovery mechanism for bots that did not check `fora.json`. The header value is a URL that the agent can `GET` to learn about available content and pricing.
...
- `protocol` -- identifies this as a FORA protocol response, not a generic 403.
...
- `fora_json_url` -- direct link to `fora.json` so the agent can discover all authorized Exchanges, not just the one in `X-Content-Rules`.
...
**Level 2: Per-IP rate limiting on fora.json**

fora.json is cacheable, but an aggressive agent might poll it. Rate limit to **10 requests/minute per IP** and rely on `Cache-Control` for normal operation.
...
The FORA project should maintain a community-curated bot pattern list as a JSON file at a well-known URL (e.g., `https://fora-protocol.org/bot-patterns.json`). Edge functions can periodically fetch updates from this list. The fetch happens on a timer (hourly), not per-request.
