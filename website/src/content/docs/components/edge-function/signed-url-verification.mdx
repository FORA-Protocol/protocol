...
1. Its **public key**, in the `X-FORA-Agent-Key` header — raw Ed25519 key, base64url with no padding. The choice of carrier is security-irrelevant, because the thumbprint is locked by the URL signature.
...
sequenceDiagram
    ...
    Agent->>Edge: GET retrieval_endpoint<br/>X-FORA-Agent-Key + Signature-Input + Signature
...
|---|---|---|
| Edge function (Cloudflare Workers, Fastly Compute, Lambda@Edge) | Yes, by default | Full steps 1-4. FORA reference implementations run here. |
...
Akamai is not a supported target. EdgeAuth is a symmetric token scheme built on a secret shared with the CDN, which is the model FORA deliberately does not use.
