...
graph LR
    subgraph "CloudFront Distribution"
        CF_FUNC["CloudFront Function<br/>(viewer request)<br/>Bot detection + fora.json + rsl.txt<br/>Signed URL passthrough"]
...
graph LR
    subgraph "Cloudflare Edge"
        WORKER["Cloudflare Worker<br/>Bot detection<br/>fora.json + rsl.txt serving<br/>HMAC signature verification<br/>Agent identity binding"]
...
- The 4ms execution limit is tight. Bot detection (string matching) and fora.json serving fit. HMAC verification with Web Crypto fits. HTTP subrequests do not fit in 4ms.
...
graph LR
    subgraph "Akamai Edge"
        EW_CLIENT["EdgeWorker: onClientRequest<br/>Bot detection<br/>fora.json + rsl.txt serving<br/>EdgeAuth passthrough"]
...
graph LR
    subgraph "Fastly Edge"
        COMPUTE["Compute Service (WASM)<br/>Bot detection<br/>fora.json + rsl.txt serving<br/>HMAC/RSA verification<br/>Agent identity binding<br/>Single-use enforcement"]
...
| fora.json + rsl.txt serving | Yes | Yes | Yes | Yes | Yes |
