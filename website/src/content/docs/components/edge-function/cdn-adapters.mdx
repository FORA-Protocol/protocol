...
graph LR
    subgraph "Cloudflare Edge"
        WORKER["Cloudflare Worker<br/>Bot detection<br/>fora.json + rsl.txt serving<br/>Ed25519 signature verification<br/>Agent identity binding"]
...
with the CDN, which is the model FORA deliberately does not use: every FORA delivery
endpoint verifies with a public key and holds nothing secret. Running FORA on Akamai would
...
graph LR
    subgraph "Fastly Edge"
        COMPUTE["Compute Service (WASM)<br/>Bot detection<br/>fora.json + rsl.txt serving<br/>Ed25519 signature verification<br/>Agent identity binding<br/>Single-use enforcement"]
...
| fora.json + rsl.txt serving | Yes | Yes | Yes | Yes |
