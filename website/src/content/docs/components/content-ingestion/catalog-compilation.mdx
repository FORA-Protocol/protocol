...
| `PricingModel` | `fora.Pricing` | `model` | Enum mapping |
| `Rate` | `fora.Pricing` | `rate` | Decimal |
| `Currency` | `fora.Pricing` | `currency` | ISO 4217 |
| `EstimatedTokens` | `fora.Pricing` | `estimated_quantity` | `WordCount * 1.32` |
| `Rate / EstimatedTokens` | `fora.Pricing` | `unit_cost` | Computed at compilation |
| `Domain + Path` | `fora.ResourceIdentity` | `canonical_url` | Full URL |
| `Permissions` | `fora.Restriction` (kind `RESTRICTION_KIND_FUNCTION`, on `LicenseTerm.restrictions`) | `permitted` | From RSL permits (hyphenated tokens, e.g. `ai-input`) |
| `Prohibitions` | `fora.Restriction` (kind `RESTRICTION_KIND_FUNCTION`, on `LicenseTerm.restrictions`) | `prohibited` | From RSL prohibits (hyphenated tokens, e.g. `ai-train`) |
...
|
+-- NO -> Did RSL or FORA sitemap provide pricing hints?
