...
description: "The FORA Broker is a demand-side router that queries multiple Exchanges, compares offers, enforces budget, and selects the best resource deal for AI agents."
---
...
func (o *Broker) Execute(ctx context.Context, offer *forav1.Offer, req *forav1.DiscoveryRequest) (*forav1.TransactionResponse, error) {
...
FORA uses a two-layer signing model: the **agent's request signature** proves who originated the request, and the **forwarding signature stack** proves which intermediaries forwarded it.
...
sequenceDiagram
    ...
    Note over Orch: Selection Engine:<br/>5 offers received<br/>4 carry a term the agent can honour (Offer.terms[0].restrictions)<br/>3 pass RequestConstraints<br/>2 after dedup (1 AP article from both)<br/>Rank by unit_cost: MP-A offer leads
