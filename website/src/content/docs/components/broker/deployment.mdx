...
description: "Deployment models, configuration reference, caching strategy, failure modes, and scaling considerations for the FORA Broker."
---
...
graph LR
    subgraph "Model A: Agent SDK (embedded library)"
        AgentSDK[AI Agent<br/>+ ForaClient] --> MPAS[Exchanges]
    end
...
func main() {
    ...
    path, handler := forav1connect.NewBrokerServiceHandler(orch)
...
func main() {
    ...
    mux := http.NewServeMux()
    path, h := forav1connect.NewBrokerServiceHandler(handler)
...
func (m *multiTenantBroker) Resolve(
    ctx context.Context,
    req *connect.Request[forav1.DiscoveryRequest],
) (*connect.Response[forav1.DiscoveryResponse], error) {
...
exchanges:
  ...
  - domain: exchange.ssp-beta.com
    endpoint: https://exchange.ssp-beta.com/fora/v1
    trust: verified
    priority: 2

# fora.json crawling (automated discovery)
discovery:
  enabled: true
  domains:
    - techcrunch.com
...
graph LR
    subgraph "Discovery Sources"
        ...
        ParseJSON[fora.json Crawling<br/>provider domains]
...
**Source 2: fora.json crawling.** The Broker periodically crawls `/.well-known/fora.json` from provider domains to discover authorized Exchanges.

**Source 3: Background discovery on first request.** When the agent requests a URI for an unknown domain, the Broker background-crawls that domain's fora.json. The current request is NOT blocked.
...
|---|---|---|
| `discovered` | Found in a fora.json | DiscoverResources only (comparison shopping) |
...
An attacker who poisons a provider's fora.json would only gain `discovered` trust, which allows supply queries but not financial transactions. The operator must explicitly promote to `verified` before the Broker will spend money there.
...
| fora.json per domain | 1 hour (configurable) | Background goroutine |
...
type ExchangeRegistry interface {
    Resolve(constraints *forav1.RequestConstraints) []ExchangeEntry
...
graph TB
    subgraph "Broker Caches"
        C1[fora.json Cache<br/>per provider domain<br/>TTL: 1 hour]
...
|---|---|---|---|
| **fora.json** | Provider domain | 1 hour | fora.json changes rarely. Matches `max-age` from the provider's HTTP cache headers if present. |
...
**All Exchanges timeout.** The Broker returns a Connect error with code `UNAVAILABLE`. The agent can retry, query Exchanges directly, or fall back to non-FORA content access.
...
type ReportingRelay interface {
    Track(txn *forav1.TransactionResponse, exchange string)
    Forward(ctx context.Context, report *forav1.UsageReport) (*forav1.UsageReportResponse, error)
