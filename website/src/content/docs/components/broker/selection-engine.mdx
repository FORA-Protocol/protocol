...
type ClientPool struct {
    mu      sync.RWMutex
    clients map[string]forav1connect.ExchangeServiceClient
}

func (p *ClientPool) GetOrCreate(mp ExchangeEntry) forav1connect.ExchangeServiceClient {
    p.mu.RLock()
    ...
    client := forav1connect.NewExchangeServiceClient(httpClient, mp.Endpoint)
...
```mermaid
graph LR
    A[Collect all Offers<br/>from N Exchanges] --> B[One offer per term<br/>agent chooses an honourable offer]
...
type SelectionEngine interface {
    ...
    Select(
        ...
        constraints *forav1.RequestConstraints,
...
## Stage 1: Surface Offer Restrictions (Agent Chooses an Offer)

Restrictions ride on the offer, not on a broker-side filter. Each offer's single `terms[0]` entry is a `LicenseTerm` carrying `repeated Restriction` (with kinds like `RESTRICTION_KIND_FUNCTION`, `RESTRICTION_KIND_GEOGRAPHY`, `RESTRICTION_KIND_USER_TYPE`). The Broker does **not** match a requester's self-declared attributes (function, geography, user type) against these restrictions to drop offers. Instead it surfaces the restrictions carried on each offer's own term; the agent chooses the offer whose restrictions it can honour. Real enforcement happens downstream at accept -> report -> reconcile.
...
func honourableTerm(offer *forav1.Offer, declaredFunction string) *forav1.LicenseTerm {
...
func restrictionsHonoured(restrictions []*forav1.Restriction, declaredFunction string) bool {
    for _, r := range restrictions {
        if r.Kind != forav1.RestrictionKind_RESTRICTION_KIND_FUNCTION {
...
```go
func filterByConstraints(offers []rankedOffer, c *forav1.RequestConstraints, budget BudgetTracker) []rankedOffer {
...
func contentIdentityKey(id *forav1.ResourceIdentity) string {
    if id == nil {
...
```go
func (e *defaultEngine) rankOffers(offers []rankedOffer, constraints *forav1.RequestConstraints) []rankedOffer {
    for i := range offers {
        score := 0.0
...
```go
func verifyOfferSignature(offer *forav1.Offer, exchange string, pubKeys map[string]ed25519.PublicKey) error {
    if offer.Signature == "" {
...
type SelectionPolicy interface {
    ...
    Rank(offers []rankedOffer, constraints *forav1.RequestConstraints) []rankedOffer
}
...
Before executing a transaction, the Broker SHOULD verify that the Exchange is authorized by the provider whose content is being licensed. This is the FORA equivalent of ads.txt verification in programmatic advertising.
...
func (o *Broker) verifyExchangeAuthorization(
    ...
    if err != nil {
        return false, fmt.Errorf("fetching fora.json for %s: %w", contentDomain, err)
    }
...
type SelectionLog struct {
    ...
    FORARequestID       string              `json:"fora_request_id"`
...
The Broker is an intermediary. FORA addresses transparency through:
