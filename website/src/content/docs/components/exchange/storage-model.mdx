...
**Authentication**: Provider authenticates via a separate admin credential (not the FORA protocol). The Exchange SHOULD enforce domain ownership verification (e.g., DNS TXT record or `fora.json` presence) before granting audit access.
