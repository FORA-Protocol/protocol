...
The Exchange Node is the core transaction component in the FORA architecture. It is a single Go binary (using Connect-Go for gRPC/HTTP) that acts as the commercial intermediary between AI agents requesting resources and providers offering them. A single Exchange Node serves many providers — one Exchange operator runs an Exchange that serves 10, 100, or 1000+ providers.

The Exchange implements the FORA v1.0 `ExchangeService` (4 RPCs: `DiscoverResources`, `ExecuteTransaction`, `ReportUsage`, `DisputeTransaction`).
...
| **[Exchange Manifest](/protocol/exchange-manifest/)** | Machine-readable self-description at `/.well-known/fora.json` (`role=ROLE_EXCHANGE`). Declares endpoints, capabilities, accepted verifiers, `supported_profiles`, and `base_currency`. Enables Broker capability negotiation and cross-exchange price comparison |
...
| **Extension Profiles** | Declares conformance to domain extension profiles (e.g., `fora-news-v1`, `fora-academic-v1`, `fora-legal-v1`) via `supported_profiles` in the manifest. Includes profile-specific ext fields in offers |
...
type OfferSigner interface {
    Sign(offer *forav1.Offer) (string, error)
    VerifyAndDecode(offerID string, signature string, algorithm string) (*forav1.Offer, error)
}
