...
type TenantConfig struct {
    ...
    PricingOverrides map[string]*forav1.Pricing // path pattern -> pricing
    ...
    // Reporting policy
    ReportingPolicy *forav1.ReportingObligation
...
3. The provider dropping `fora.json` pointing to this Exchange
...
The edge function expects to fetch `fora.json` (WellKnownManifest, `role=ROLE_PUBLISHER`) and `rsl.txt` from the Exchange on behalf of provider domains. The Exchange generates these from its tenant catalog and serves them for edge caching.

### Endpoints

```
GET /provider/{domain}/fora.json
```
