...
sequenceDiagram
    ...
    Note over SD: One Offer per entitled LicenseTerm<br/>(restrictions ride on that offer's term; the agent chooses an offer, no server-side filter)
...
func (h *ExchangeHandler) ExecuteTransaction(
    ...
    if blocked, reason := h.reporting.CheckBuyerCompliance(ctx, account.ID); blocked {
        ...
        return connect.NewResponse(denyAllItems(req, forav1.DenialReason_DENIAL_REASON_REPORTING_OVERDUE)), nil
    }
    ...
    for _, item := range req.Msg.Items {
        ...
        deny := func(reason forav1.DenialReason) (*connect.Response[forav1.TransactionResponse], error, bool) {
            if singleItem {
        ...
        if err != nil {
            if resp, denyErr, done := deny(forav1.DenialReason_DENIAL_REASON_SIGNATURE_INVALID); done {
                return resp, denyErr
            }
            continue
        }
        ...
        if err != nil {
            if resp, denyErr, done := deny(forav1.DenialReason_DENIAL_REASON_CONTENT_UNAVAILABLE); done {
                return resp, denyErr
            }
            continue
        }
    ...
    resp := &forav1.TransactionResponse{
        ...
        AgentIdentityHash: agentHash,
        Items:             resultItems,
        TotalCost:         &forav1.Cost{Amount: totalAmount, Currency: currency},
    }
...
func transactionDenied(reason forav1.DenialReason, mismatches ...forav1.RestrictionKind) error {
...
**Rejection reasons** come from [`CatalogRejectionReason`](/reference/proto-fora/#catalogrejectionreason). The ones this flow can produce are `CATALOG_REJECTION_REASON_NOT_CATALOG_CONTRIBUTOR` (the caller is not authorized for the domain), `CATALOG_REJECTION_REASON_SIGNATURE_INVALID`, `CATALOG_REJECTION_REASON_DOMAIN_NOT_VERIFIED` and `CATALOG_REJECTION_REASON_MALFORMED_ENTRY`, which the envelope rules make the common one. The attestation-specific failures — a bad attestation signature, an unknown verifier, oversized claims — have no dedicated value in that enum; they travel in the refusal's human-readable message.
