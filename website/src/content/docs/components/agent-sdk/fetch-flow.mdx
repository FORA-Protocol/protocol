...
| `NoExchangeError` | Retry after TTL, or configure Exchange manually | Content not available via FORA |
| `NoOfferError` | None -- content genuinely unavailable | Caller falls back to non-FORA access |
...
| `EndpointRefused` (`EndpointRefusedError`, `ErrEndpointRefused`) | **None -- terminal.** The Exchange answered and the answer is unusable: it advertises an endpoint on a host or port that did not serve its manifest, or one carrying userinfo. Retrying re-reads the same manifest | Content not available via this Exchange until its operator fixes `fora.json` |
...
```go
import "example.com/fora-agent-sdk/testutil" // planned; not a shipped package
...
func TestAgentLogic(t *testing.T) {
    ...
    client, _ := fora.NewClient(fora.Config{
        ...
        SigningKey:  "test-key",
        Budget:      fora.Budget{MaxPerRequest: 1.00, Currency: "USD"},
        Exchanges: []fora.ExchangeConfig{{
            Domain:   "mock.exchange.test",
            Endpoint: mock.URL(),
            Trust:    fora.TrustVerified,
        }},
        Discovery: fora.DiscoveryConfig{AutoDiscover: false},
    })
