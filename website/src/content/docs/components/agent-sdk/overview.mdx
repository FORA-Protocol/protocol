...
::example{file="sdk/go/examples/agent/agent.go" regions="imports,client,discover,execute,fetch,report" lang="go"}
...
::example{file="sdk/ts/examples/agent.ts" regions="imports,client,discover,execute,fetch,report" lang="typescript"}
...
::example{file="sdk/python/README.md" fence="agent-example" lang="python"}

That block is the [Python README](https://github.com/FORA-Protocol/protocol/blob/main/sdk/python/README.md)'s
example, pulled in from that file rather than copied. The package's own test suite
extracts the same block and runs it against an in-process Exchange, so a name that stops
resolving fails a test.
