...
The FORA Agent SDK is a library that agent developers embed directly in their application process. It provides one-liner content access -- a single `Fetch(url)` call that handles discovery, negotiation, transaction, delivery, budget enforcement, and usage reporting behind the scenes.
...
// NewClient creates a FORA client with the given configuration.
...
```go
client, err := fora.NewClient(fora.Config{
...
import { ForaClient } from "@fora-protocol/agent-sdk";

const client = new ForaClient({
    agentId: "research-bot",
    agentDomain: "agent.example.com",
    signingKey: process.env.FORA_SIGNING_KEY!,
...
from fora_sdk import ForaClient, Budget

client = ForaClient(
    agent_id="research-bot",
    agent_domain="agent.example.com",
    signing_key=os.environ["FORA_SIGNING_KEY"],
...
| TypeScript | [`gen/ts/`](https://github.com/FORA-Protocol/protocol/tree/main/gen/ts/) (types) + [`sdk/ts/`](https://github.com/FORA-Protocol/protocol/tree/main/sdk/ts/) (hand-written SDK: `src`, `core`, `resolvers`, `client`, `hono`) | Shipped and CI-gated; npm package not yet published | `zod` + `undici` + `ajv` + `canonicalize` (`hono` optional) |
| Python | [`gen/python/`](https://github.com/FORA-Protocol/protocol/tree/main/gen/python/) (types) + [`sdk/python/`](https://github.com/FORA-Protocol/protocol/tree/main/sdk/python/) (hand-written SDK: `fora_sdk`, `fora_sdk.core`, `fora_sdk.resolvers`, `fora_sdk.client`, `fora_sdk.sync`) | Shipped and CI-gated; PyPI package not yet published | `pydantic` + `httpx` + `cryptography` |
