---
title: "FORA for Exchange Operators"
description: "How to operate a FORA Exchange — aggregate resource catalogs, handle transactions, earn a margin"
---
...
FORA was validated across **18 real-world scenarios** spanning 10 industries. The same protocol works for all of them:
...
FORA pricing uses three fields: `unit_cost` (price per unit), `unit` (what's being metered), and `estimated_quantity` (how much the agent will consume). No assumption about what's being metered:
...
// Legal exchange
{ "supported_profiles": ["fora-legal-v1"] }

// Multi-domain exchange
{ "supported_profiles": ["fora-news-v1", "fora-academic-v1"] }
```

Agents declare which profiles they understand. The Broker routes queries to Exchanges that support the requested profile. An academic research agent requesting `fora-academic-v1` is routed to your Exchange if you declare that profile.
...
|---|---|---|
| `fora-news-v1` | News, podcasts, broadcasting | Set IPTC subject codes, correction status, podcast metadata in offer ext fields |
| `fora-academic-v1` | Scholarly articles, preprints | Set DOI, OA status, version lifecycle in offer ext fields |
| `fora-legal-v1` | Legislation, case law, patents | Set jurisdiction, amendment chains, ELI/ECLI in offer ext fields |

A Exchange MAY implement zero, one, or multiple profiles. You can start with one domain and expand.

## If You're an SSP: How FORA Maps to Your Infrastructure

If you operate an SSP or ad exchange, FORA maps directly to your existing concepts:

```
Ad-Tech Concept          →  FORA Equivalent
─────────────────────────────────────────────
SSP / Exchange           →  Exchange Node
Publisher                →  Resource Provider
ads.txt                  →  fora.json (authorized providers)
...
5. If applicable, deploy the FORA edge function to their delivery endpoint (CDN, API gateway, PACS server) for signed URL verification
...
You don't need bilateral deals with every AI company. Any agent that speaks the FORA protocol can buy from your Exchange.
...
FORA defines identity- and entitlement-related denial reasons your Exchange MUST return when appropriate. The delegation/scope reasons:
...
This is the identity/entitlement subset with operator guidance; the complete, authoritative `DenialReason` enum is rendered from the proto on the [proto reference](/reference/proto-fora/). For the full identity model specification — Requester fields, the JWT chain, scope format, and key management — see [Authentication](/protocol/authentication/).
...
FORA uses a single unit-agnostic pricing model, `PRICING_MODEL_PER_UNIT`, for all metered resources. `Pricing.unit` names what is metered, so non-text resources are priced through the same model:
...
FORA supports `DELIVERY_METHOD_STREAMING` for real-time connections (WebSocket, SSE, HLS, Icecast). Your Exchange generates a signed URL pointing to a streaming endpoint. The agent connects and receives continuous data. Stream mechanics (heartbeat, reconnection, concurrent connection limits) are managed by the provider, not the protocol.

Streaming transactions follow the same FORA flow, but the signed URL has a longer TTL (streaming sessions last minutes/hours) and usage is reported in time-based units (minutes, hours).
