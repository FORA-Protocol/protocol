---
title: "FORA for Resource Providers"
description: "How any organization with digital resources can expose them through FORA — discover, price, deliver, and get paid for every AI access"
---
...
FORA gives you a standard way to **expose, price, and deliver any digital resource** to any AI agent through any Exchange — one protocol, not a hundred integrations.

## What FORA Does for You

FORA turns your resources into products that AI agents can discover, evaluate, purchase, and verify. Here's how:
...
|---|---|---|---|
| News publisher | Articles, podcasts | CDN edge function blocks bots, redirects to Exchange | Edge function + `fora.json` |
...
If your resources are served through a CDN — news articles, editorial content, research papers hosted on your site — deploy the FORA edge function. This is the standard approach for CDN-hosted resources.

**Nothing to deploy if your Exchange operator supports FORA.** Your Exchange operator deploys the CDN edge function on your behalf and manages everything. You sign a contract, grant CDN access, done.

If you self-manage, deploy using the [Terraform module](https://github.com/FORA-Protocol/protocol/tree/main/terraform/aws-fora-edge/):
...
module "fora_edge" {
  ...
  exchange_endpoint       = "https://exchange.your-ssp.com/fora/v1"
...
- Serves a `fora.json` file declaring your Exchange relationships
...
2. Accepts FORA transactions from agents
3. Calls the D&B API on the agent's behalf
4. Delivers the report through a signed URL
5. Reports usage back through the FORA billing chain

The provider's existing API doesn't change. The Exchange handles the FORA protocol surface.

**Example: EUR-Lex Legislation**

A government agency like EUR-Lex can offer legislation through FORA with tiered pricing:
...
The Cancer Imaging Archive (TCIA) hosts thousands of imaging studies (CT, MRI, PET). Through FORA:
...
FORA classifies your resources into three mutability levels. This drives how agents verify what they receive and how disputes work.
...
FORA v1.0 uses domain extension profiles — standardized metadata vocabularies for specific industries. If your resources fall into one of these domains, your Exchange can enrich offers with profile-specific metadata that agents understand.

| Profile | Scope | Key Metadata | Standards |
|---|---|---|---|
| **fora-news-v1** | News articles, podcasts, broadcasting | IPTC subject codes, correction/retraction status, transcript linking | IPTC NewsML-G2, Podcasting 2.0, W3C TDMRep |
| **fora-academic-v1** | Scholarly articles, preprints, datasets | DOI, open access status, version lifecycle (preprint/AAM/VoR) | CrossRef, OpenAlex, COUNTER 5.1, CrossMark |
| **fora-legal-v1** | Legislation, case law, patents | Jurisdiction, amendment chains, ELI/ECLI identifiers, patent families | ELI, CELEX, ECLI, Akoma Ntoso, WIPO ST.3 |
...
**No profile for your domain?** That's fine. FORA's core protocol is domain-agnostic — pricing, delivery, attestation, and dispute all work without any extension profile. Profiles add richer metadata for agents that understand them, but they're not required. Data providers (credit, geospatial, pharma, medical) work with FORA's core fields today. As communities develop, new profiles can be proposed through the extension governance process.

## The `fora.json` Provider Manifest

Every provider with a web presence can host a `fora.json` file at their domain root (like `ads.txt` for advertising). This file declares:
...
Your public verification keys are published separately in your WBA directory (the JWK Set at `/.well-known/http-message-signatures-directory`), not in `fora.json`.

AI agents and Brokers read `fora.json` to discover how to access your resources through FORA.
...
Those continue as subscription deals within FORA. The agent gets access at $0/request under the subscription, and the Exchange tracks usage against the deal terms.

**Does this only work for web content?**
No. FORA handles any digital resource — articles, credit reports, court filings, medical imaging, satellite data, drug databases, live feeds. The protocol is resource-agnostic. Web content served through a CDN is one deployment model; API-backed data products are another.

**What about my robots.txt?**
Keep it. `robots.txt` blocks training crawlers. FORA handles real-time inference access (the agent needs a resource right now to complete a task). Different traffic, different treatment.
...
FORA's core protocol works without profiles. You get discovery, pricing, transactions, delivery, attestation, and disputes out of the box. Profiles add domain-specific metadata richness but aren't required.

## Next Steps

1. **Web-facing content providers**: Ask your Exchange operator if they support FORA. If yes, they deploy everything. If no, [self-service deployment](/architecture/deployment-models/) takes 15 minutes.
