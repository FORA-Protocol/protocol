...
description: "The current state of AI metered resource access: standards ecosystem, closed platforms, and where FORA fits in the timeline."
---
...
### FORA (Resource Access Metering Protocol)

The open transaction protocol that completes the stack.

- Adds discovery (`fora.json`), transaction execution, signed URL delivery, and usage reporting
...
AI resource access is at the same inflection point. Microsoft PCM and TollBit are building walled gardens -- the equivalent of pre-OpenRTB proprietary exchanges. FORA aims to be the OpenRTB for resource access: an open protocol that any Exchange, AI company, or provider can implement.

## Comparison

| Layer | RSL | CoMP | Microsoft PCM | TollBit | FORA |
|---|---|---|---|---|---|
| Terms declaration | rsl.txt | Package object | Provider dashboard | Provider dashboard | RSL ingestion by Exchange |
| Discovery | File on domain | Not defined | Microsoft index | TollBit API | `fora.json` + edge function fallback |
...
FORA v1.0 prevents protocol fragmentation through a three-layer governance model:
...
Three domain extension profiles ship with v1.0: `fora-news-v1` (news, podcasts, broadcasting), `fora-academic-v1` (scholarly articles, preprints, datasets), and `fora-legal-v1` (legislation, case law, patents). Each profile is grounded in existing industry standards (IPTC NewsML-G2, CrossRef/OpenAlex, ELI/ECLI/Akoma Ntoso).

Exchanges declare conformance via `supported_profiles` in their manifest. Agents declare which profiles they understand in the request. The Broker uses these declarations for routing -- a legal research agent requesting `fora-legal-v1` is routed to Exchanges that support that profile.

## Scope Boundary

FORA v1.0 draws a clear boundary: **resource access is in scope; job execution is not.**

The test: "Does the resource exist before the transaction?" If yes, it is a FORA use case. If the transaction creates something new from agent input (translation, ML inference, document generation), it is not.
...
**Out of scope:** translation, ML inference, document generation. A Exchange offering these should use a different protocol. However, a pre-computed translation database (translations as resources) is a valid FORA use case.

## The Prebid LLM Group

FORA originated from the Prebid LLM Content Monetization Committee, an industry group within Prebid (the open-source header bidding platform). The committee is in its early stages, with no working implementations yet beyond FORA.
...
- FORA leverages the same architectural patterns (multi-exchange competition, real-time bidding, pluggable adapters)
...
| March 2026 | FORA v1.0: metering generalization, streaming delivery, content mutability, extension profiles, CoMP alignment |
...
1. **IAB CoMP is being defined now** -- the public comment period closes April 2026. The industry is actively shaping the data model. FORA builds on CoMP, so alignment matters.
...
- [What Is FORA?](/getting-started/what-is-fora/) -- protocol overview and how it works
- [Standards Layering](/protocol/standards-layering/) -- technical detail on how RSL, CoMP, and FORA interoperate
- [Discovery Paths](/protocol/discovery-paths/) -- how agents find and enter the FORA flow
