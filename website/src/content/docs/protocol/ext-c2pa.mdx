...
FORA-C2PA v1 integrates the C2PA (Coalition for Content Provenance and Authenticity) standard into FORA, enabling agents and Exchanges to:

- **Verify** content provenance — who created it, what tools were used, whether it's been modified
- **Enforce** creator-declared training/mining restrictions commercially via FORA access controls
...
C2PA proves **what content IS** (provenance). FORA manages **what you can DO with it** (commercial access). This profile bridges the two.

### Design Principle: Consume Results, Not Structures

FORA does not re-implement C2PA internals. C2PA has its own container format (JUMBF), its own PKI (X.509 certificates), and its own validation algorithm (COSE Sign1). This profile defines how **C2PA validation results** are carried in FORA's existing attestation mechanism — not how to parse JUMBF or verify COSE signatures.

A **C2PA verification vendor** (the Exchange itself, or a third party like Truepic or a C2PA Trust List validator) validates the manifest and publishes results as a standard FORA `ResourceAttestation`. C2PA-unaware agents benefit from the results without needing C2PA libraries.

### Who Should Implement This

- **Exchanges** serving content with C2PA credentials (news photos, AI-generated images, medical imaging, legal documents)
- **Verification vendors** that validate C2PA manifests and bridge the results into FORA attestations
- **Agents** that need provenance assurance for high-stakes content (medical, legal, financial)
- **Providers** that embed C2PA credentials in their content and want FORA to enforce their training/mining declarations

### Prerequisites

Implementers must be familiar with:

- FORA protocol (`proto/fora/v1/fora.proto`)
...
An Exchange declares support by including `"fora-c2pa-v1"` in `WellKnownManifest.supported_profiles` (`role=ROLE_EXCHANGE`):

```json
{
  "ver": "1.0",
  "domain": "exchange.example.com",
  "supported_profiles": ["fora-c2pa-v1"],
...
These fields are in the core `fora.proto` (Layer 1), not in `ext`:
...
When a C2PA verification vendor validates a manifest, it publishes results as a FORA `ResourceAttestation` with these standardized claim names:
...
The Exchange SHOULD automatically map C2PA training/mining declarations to FORA access restrictions:

| C2PA Assertion | FORA Mapping |
|----------------|-------------|
...
1. **Declaration layer** (C2PA): The content creator embeds their intent in the content itself. This travels with the content, even outside FORA channels.
2. **Term layer** (FORA): The Exchange maps the C2PA declaration onto `LicenseTerm.restrictions` that ride on the offer. The agent chooses the offer whose term it can honour and bears compliance (enforced downstream at accept → report → reconcile). As a convenience, an Exchange MAY pre-filter — an agent that volunteers `acceptable_restrictions: [{ axis: "RESTRICTION_KIND_FUNCTION", values: ["ai-train"] }]` on the query can ask to skip offers for content where training is prohibited so it doesn't pay to discover them — but that filter is advisory, not an enforcement guarantee.

---

## Dispute Enhancement

C2PA content bindings strengthen FORA's dispute resolution:
...
C2PA and FORA use different PKI:
...
```
C2PA World                          FORA World
─────────────────                   ──────────────────
X.509 cert chain                    Ed25519 JWKS
COSE Sign1 signature    ──►        detached Ed25519 signature
C2PA Trust List          Vendor     FORA accepted_verifiers
JUMBF manifest          bridges     ResourceAttestation.claims
RFC 3161 TSA                        attested_at timestamp
```

The vendor's Ed25519 key (published in its WBA directory, the JWK Set at `/.well-known/http-message-signatures-directory`, and referenced by the attestation's RFC 7638 thumbprint `keyid`) attests to the results of X.509/COSE validation. FORA has no dedicated verifier role; a verifier publishes under its operator's role (`ROLE_PUBLISHER` for a self-attesting publisher, `ROLE_EXCHANGE` for a standalone vendor). FORA participants trust the vendor; the vendor trusts the C2PA Trust List. One-way bridge, clean separation.
...
- Declare `"fora-c2pa-v1"` in `WellKnownManifest.supported_profiles`
...
Both C2PA and FORA support Ed25519/EdDSA. A verification vendor that holds both an X.509 certificate (for C2PA signing) and an Ed25519 key (for FORA attestation signing) can participate in both ecosystems.

| Algorithm | C2PA Support | FORA Support |
|-----------|-------------|-------------|
...
- **fora-news-v1**: News photos with C2PA credentials benefit from both profiles. The news profile tracks editorial metadata (corrections, retractions); the C2PA profile tracks content provenance (camera, edits, creator identity). They compose — an offer can carry both `news.*` and `c2pa.*` claims.
- **fora-comp-v1**: CoMP's `Video.c2pa`, `Image.c2pa`, and `Audio.c2pa` fields carry C2PA manifest URIs at the media asset level. This profile operates at the FORA resource level — the `ResourceIdentity.c2pa_manifest` is the canonical pointer, independent of CoMP.
