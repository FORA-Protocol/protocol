---
title: "Extension: fora-academic-v1"
description: "Scholarly articles, preprints, and datasets extension profile — field vocabulary, conformance levels, and examples"
---

## Overview

FORA-Academic v1 defines the standardized `ext` field vocabulary for scholarly and academic content traded through the FORA protocol. It covers journal articles, conference papers, book chapters, preprints, and datasets.
...
- **Providers** publishing scholarly content who want their articles discoverable and transactable through FORA
...
An Exchange declares support for this profile by including `"fora-academic-v1"` in its `WellKnownManifest.supported_profiles` array (`role=ROLE_EXCHANGE`). The manifest is served at `/.well-known/fora.json` (`role=ROLE_EXCHANGE`).
...
Academic articles exist in multiple versions that differ in content, formatting, and legal status. FORA-Academic models these using existing NISO and CrossRef conventions.
...
|----------------------|---------------------|--------------|-------------|----------------------------|
| `"gold"` | No (full OA journal) | Yes (CC-BY typical) | Usually yes | Free to access; FORA Exchange MAY offer at `PRICING_MODEL_FREE` |
...
Academic content access often depends on institutional subscriptions. FORA models this through the existing `Offer.subscription_id` field, extended with academic-specific context.
...
FORA's `ReportUsage` RPC handles per-transaction usage reporting. For Exchanges that also need COUNTER 5.1-compatible aggregate reporting, the following `UsageReport.ext` fields map FORA transactions to COUNTER vocabulary.

| Key | Type | Required | Source Standard | Description |
|-----|------|----------|-----------------|-------------|
| `academic.counter_metric_type` | string | OPTIONAL | COUNTER 5.1 | COUNTER metric type for this transaction. Values: `"Total_Item_Requests"` (content downloaded/viewed), `"Unique_Item_Requests"` (unique items per session), `"Total_Item_Investigations"` (metadata viewed). Most FORA transactions map to `"Total_Item_Requests"`. |
...
| `academic.counter_access_method` | string | OPTIONAL | COUNTER 5.1 | Access method. Values: `"Regular"` (standard human or agent use), `"TDM"` (text and data mining). FORA transactions for AI input SHOULD use `"TDM"`. |

**Exchange action:** Aggregate `UsageReport` data with these COUNTER fields to produce COUNTER 5.1-compliant Title Reports (TR) and Item Reports (IR) for institutional subscribers. This enables libraries to see FORA-mediated usage alongside traditional usage in their COUNTER reports.
