...
description: "How FORA classifies resource stability (static, dynamic, live) to drive hash verification, freshness filtering, and dispute behavior"
---
...
Without a mutability signal, the agent cannot distinguish "content was tampered with" from "content was updated since the offer was generated." FORA solves this with the `ResourceMutability` enum — set on the Offer's `ResourceIdentity.resource_mutability` and declarable at ingest via `ResourceEntry.resource_mutability` — and two freshness fields: `Offer.data_as_of` and `RequestConstraints.max_data_age`.
...
- [Proto: FORA v1](/reference/proto-fora/) -- full `ResourceMutability` enum and field definitions
