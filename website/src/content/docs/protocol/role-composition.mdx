...
description: "How a single operator can run multiple FORA roles (Agent, Broker, Exchange, Publisher) under one corporate root without breaking the protocol's trust model."
---

## Roles are compositional, not exclusive

FORA defines four roles via the `Role` enum in `WellKnownManifest`:
...
Operators that already span the demand and supply sides of digital content distribution map naturally onto multiple FORA roles. Typical patterns:

| Operator surface | FORA role | `WellKnownManifest` domain |
|---|---|---|
