...
`comp.proto` mirrors the removal — there is no `revshare` field to carry. FORA
...
An Exchange declaring `fora-comp-v1` support MUST:
...
An Agent using `fora-comp-v1` MUST:
- Include `comp.function` in Usage when reporting

## Relationship to Core Protocol

The core FORA protocol carries universal fields:
...
The `fora-comp-v1` profile adds CoMP-specific semantics:
