---
title: "FORA v1.0 Walkthrough"
description: "End-to-end walkthrough of FORA v1.0 with two scenarios: per-article access (no subscription) and subscription access with delegation (JWT). Shows identical SDK code for both paths."
---

> Complete walkthrough of FORA v1.0 as of March 2026. Every field name, message structure, and authentication mechanism matches the current `fora.proto`. Two scenarios demonstrate how the same protocol handles pay-per-article and subscription access.

## Two Scenarios, One Protocol

This walkthrough covers two real-world scenarios that exercise different parts of the FORA v1.0 protocol:
...
The agent resolves the target domain's FORA configuration. Every FORA-enabled domain publishes a machine-readable manifest at `/.well-known/fora.json`:

```bash
curl -s https://techcrunch.com/.well-known/fora.json | jq
```
  ...
  "exchanges": [
    {
      ...
      "endpoint": "https://exchange.ssp-alpha.com/fora/v1",
...
`POST https://exchange.ssp-alpha.com/fora/v1/fora.v1.ExchangeService/DiscoverResources`
...
The Exchange looks up the article in its catalog and returns one offer for each term the requester is entitled to. It does not filter those offers against the query's `acceptable_restrictions` — the restrictions ride on each offer's own term, and the agent chooses the offer whose `RESTRICTION_KIND_FUNCTION` permitted list covers its intended use (`ai-input`); enforcement happens downstream at accept → report → reconcile:
...
`POST https://exchange.ssp-alpha.com/fora/v1/fora.v1.ExchangeService/ExecuteTransaction`
...
Same pattern as Scenario A. The agent resolves MarketData's FORA manifest:

```bash
curl -s https://marketdata.example.com/.well-known/fora.json | jq
```
  ...
  "exchanges": [
    {
      ...
      "endpoint": "https://exchange.marketdata-data.com/fora/v1",
      "relationship": "PROVIDER_RELATIONSHIP_DIRECT"
    }
  ],
  "supported_profiles": ["fora-finance-v1"]
}
```
...
`POST https://exchange.marketdata-data.com/fora/v1/fora.v1.ExchangeService/DiscoverResources`
...
Both scenarios use identical agent code. The `Requester` message carries the
delegation when there is one, and every pricing decision is the Exchange's — the agent
never branches on subscription versus pay-per-article.

The delegation is set once, on the `Requester` the client sends with every call:
...
requester := &forav1.Requester{
    ...
    // Scenario A: leave Delegation nil.
    // Scenario B: attach the delegation the principal issued.
    Delegation: &forav1.Delegation{
        PrincipalDomain: "marketdata.example.com",
        PrincipalId:     "enterprise-sub-HF-001",
        Scopes:          []string{"earnings:*"},
...
client := connect.NewClient(baseURL,
    ...
    connect.WithAgentKey(agentPublic),
...
The discovery and purchase calls are then byte-for-byte the same in both scenarios.
What differs is only what comes back:

```go
found, err := client.Discover(ctx, &forav1.ResourceQuery{
    Exchange: "exchange.example",
    Uris:     []string{articleURL},
})
if err != nil {
    return err
}
tx, err := client.Execute(ctx, found.Verified()[0])
if err != nil {
    return err
}
item := tx.GetItems()[0]

// Scenario A: item.GetCost().GetAmount() is "0.05", GetSubscriptionId() is empty.
// Scenario B: item.GetCost().GetAmount() is "0", GetSubscriptionId() is
//             "enterprise-sub-HF-001", and GetSubscriptionUnitValue() carries the
//             attributed value.
```
...
|------|-----|--------------------------|---------------------------|
| 1 | — | `fora.json` discovery | `fora.json` discovery |
