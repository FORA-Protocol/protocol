...
Resource access in FORA operates at $0.01--$0.10 per transaction. A single human reviewer spending 5 minutes costs $2.50 -- 25x to 250x the transaction value. **100% of routine disputes must be automated.** Manual review is reserved for aggregate fraud patterns, not individual transactions.
...
- [Proto: FORA v1](/reference/proto-fora/) -- full `DisputeRequest`/`DisputeResponse` message reference
