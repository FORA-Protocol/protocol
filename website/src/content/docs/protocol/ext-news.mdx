---
title: "Extension: fora-news-v1"
description: "News, media, and podcast content extension profile — field vocabulary, conformance levels, and examples"
---

## Overview

FORA-News v1 defines a standardized vocabulary of `ext` field keys for news, media, and podcast content traded through FORA Exchanges. It enables agents and Brokers to:
...
- **Agents** that consume news articles, podcast episodes, or broadcast content via FORA
...
An Exchange declares support for this profile by including `"fora-news-v1"` in its `WellKnownManifest.supported_profiles` array (`role=ROLE_EXCHANGE`):
...
```json
{
  ...
  "endpoint": "https://news-exchange.example.com/fora/v1"
}
```
...
Exchanges declaring `fora-news-v1` support SHOULD populate these fields to describe their news-specific capabilities.
...
| `news.correction_propagation` | string | Optional | FORA-News | How the Exchange handles corrections from publishers. Values: `realtime` (corrections reflected within minutes), `batch` (corrections reflected on next catalog refresh), `none` (Exchange does not track corrections). |
| `news.podcast_support` | boolean | Optional | FORA-News | Whether the Exchange catalogs podcast episodes. Brokers use this to route podcast queries efficiently. |
...
This section defines how FORA core protocol features interact with news content.
...
News publishers have strong attribution requirements. The FORA core `Usage.attribution` and `AttributionDetail` messages handle this:
...
### Level 1: Basic (Minimum viable FORA-News)
