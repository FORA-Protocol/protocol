...
description: "End-to-end FORA v1.0 walkthrough: provider setup, agent request, supply discovery, transaction, content fetch, usage reporting, dispute resolution, and reconciliation"
---

> Definitive walkthrough of FORA v1.0 as of March 2026. Every field name, signature algorithm, and endpoint matches the current `fora.proto`.

## Scenario

**Agent**: Anthropic's Claude (`claude.ai`), running the FORA Agent SDK
...
**fora.json** — declares which Exchange is authorized to transact:

```json
// GET https://techcrunch.com/.well-known/fora.json
{
  "ver": "1.0",
...
// Content-Type: application/jwk-set+json
{
  ...
  "revocation_url": "https://claude.ai/.well-known/fora-invalidations.json"
}
```

The `fora.json` manifest still declares the role and contact, but the keys live in the WBA directory:

```json
// GET https://claude.ai/.well-known/fora.json
{
  "ver": "1.0",
  "role": "ROLE_AGENT",
  "domain": "claude.ai",
...
// Content-Type: application/jwk-set+json
{
  ...
  "revocation_url": "https://broker.anthropic.com/.well-known/fora-invalidations.json"
}
```
...
2. The Verge has fora.json pointing to SSP-Beta:
   {
     "ver": "1.0",
...
Step 2: ExchangeRegistry lookup
  ...
  - Background: fetch https://techcrunch.com/.well-known/fora.json
...
**Key insight**: The SDK discovers Exchanges from the provider's `fora.json`. It only finds the authorized Exchange for THAT domain. Cross-provider discovery (same article on different domains) requires either a hosted Broker that has crawled multiple providers' `fora.json`, or the agent explicitly requesting multiple URLs across providers.

---

## Phase 2: Supply Discovery (Requester Identity, Subscription Detection, Content Attestations, Ed25519 Verification)

The SDK sends `DiscoverResources` to SSP-Alpha — `POST https://exchange.ssp-alpha.com/fora/v1/fora.v1.ExchangeService/DiscoverResources`:
...
Agent -> CDN: GET https://cdn.techcrunch.com/server/premium/ai-regulation-2026.html
              ...
              &kid=vJ3xR1mQ...
              &sig=k7Qm2xR9vT4nB8aL...
              Header: X-FORA-Agent-Key: <agent public key, base64url>
...
```json
POST https://exchange.ssp-alpha.com/fora/v1/fora.v1.ExchangeService/DisputeTransaction
...
**Without Broker (SDK only)**: the agent found the article via SSP-Alpha (from `fora.json`) and got the subscription deal. It never knew The Verge also had the article at $0.07 via SSP-Beta.
