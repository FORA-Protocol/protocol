...
|---|---|---|---|
| **1. Core Proto** | Fields in `fora.proto` | Protocol version bump | `resource_mutability`, `data_as_of`, `unit_cost` |
...
Fields included in the protobuf definition (`fora.proto`). These are cross-cutting needs validated across 30%+ of use cases, affect fundamental protocol behavior, and have consistent meaning across all domains.

Examples: `ResourceMutability`, `data_as_of`, `max_data_age`, `unit_cost`, `estimated_quantity`.

### Layer 2: Standard Extensions

Needed by multiple domains but too specialized for the proto. Documented in the FORA specification. Carried in the `ext` field (a `google.protobuf.Struct`) on protocol messages. Consumers SHOULD support these when present.
...
Agent                    Broker                 Exchange
  ...
  │  ["fora-academic-v1"]    │  supported_profiles:       │  WellKnownManifest
  │ ─────────────────────►   │  ["fora-academic-v1"]      │  supported_profiles:
  │                          │ ────────────────────────►   │  ["fora-academic-v1"]
  │                          │                            │
  │                          │  ◄──── Offers with         │
...
The Exchange declares which domain extension profiles it conforms to. Published in `/.well-known/fora.json` (`role=ROLE_EXCHANGE`). The Broker reads this at discovery time to route queries to capable Exchanges.
...
Every FORA message that carries an `ext` field also carries `ext_critical` (field 90) -- a list of ext keys that the consumer **MUST** understand. If a consumer encounters a key in `ext_critical` that it does not recognize, it **MUST** reject the message.
...
FORA adopts the COSE enumeration approach: the sender lists which ext keys are critical in a given message, rather than marking criticality on the key definition itself.
...
Five extension profile specifications have been developed for FORA v1.0, each grounded in existing industry standards.
...
**Scope:** C2PA content provenance integration -- bridges C2PA Content Credentials into FORA attestations and access controls
...
| **Key capabilities** | C2PA manifest validation bridged to FORA attestations; `c2pa.training_mining` mapped to a `LicenseTerm.restrictions` `Restriction{kind: RESTRICTION_KIND_FUNCTION, prohibited: [...]}`; soft binding for post-transcoding dispute resolution; TSA-grade temporal claims |
...
C2PA proves **what content IS** (provenance, creator, edit history). FORA manages **what you can DO with it** (pricing, transaction, dispute). This profile bridges the two: a C2PA verification vendor validates the manifest using X.509/COSE and publishes the results as a FORA `ResourceAttestation` signed with Ed25519. C2PA-unaware agents benefit from the validation results and rights enforcement without needing JUMBF or COSE libraries.

See the [full fora-c2pa-v1 specification](/protocol/ext-c2pa/).

## Creating a New Profile

A domain community creates a new profile by publishing a specification that defines:

1. **Profile name** -- format: `fora-{domain}-v{version}` (e.g., `fora-pharma-v1`)
...
```json
{
  ...
  "$id": "https://fora-protocol.org/extensions/fora-example-v1/schema.json",
  "title": "FORA-Example v1 Extension Profile",
  "type": "object",
  "properties": {
    "example.field_name": {
      "type": "string",
...
- [Proto: FORA v1](/reference/proto-fora/) -- full proto reference including `supported_profiles` fields
