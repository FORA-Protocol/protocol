---
title: "Changelog"
description: "FORA protocol changelog"
---

This page contains curated public release notes. For the exhaustive maintainer
and protocol history, see [`proto/CHANGELOG.md`](https://github.com/FORA-Protocol/protocol/blob/main/proto/CHANGELOG.md).
...
major design decisions is recorded in [`docs/design-history.md`](https://github.com/FORA-Protocol/protocol/blob/main/docs/design-history.md).
...
`/.well-known/fora.json` document's `ver` has stated a consumer-side rule since the
...
resolver verdicts, and never cached. Both manifest-reading faces apply it — the
endpoint resolver, and the registration-requirements reader added later in this
section, which answers its own seam's verdict rather than this sentinel. The
well-known key resolver is exempt: it reads a JWK Set document that carries no
manifest version, and a test in each language pins that a key document carrying a
`ver` still resolves.
...
the `fora.json` endpoint resolver (`WellKnownEndpointResolver` /
...
`github.com/FORA-Protocol/protocol/sdk/go/resolvers`. No alias shim is shipped — it is
...
`X-FORA-Entitlement-Biscuit` → `X-Entitlement-Token`, and the verifier enforces that
...
- **Renamed** `FORARequest` → `DiscoveryRequest` and `FORAResponse` →
...
are split out of `fora.json` (`WellKnownManifest`) into a pure WBA key directory
...
- **Removed** the FORA-invented non-CoMP fields: `Text.authority`/`originality`,
...
- **Caching contract** — `/.well-known/fora.json` MAY be cached (minutes–hours); the `invalidation_url` body SHOULD be short/`no-store`.
...
- The `fora-c2pa-v1` extension profile bridges C2PA X.509/COSE trust into FORA Ed25519 attestations.
...
- **fora-news-v1** — articles, podcasts, broadcasting (IPTC NewsML-G2, Podcasting 2.0)
- **fora-academic-v1** — scholarly articles, preprints (CrossRef, OpenAlex, COUNTER 5.1)
- **fora-legal-v1** — legislation, case law, patents (ELI, ECLI, Akoma Ntoso)
- **fora-comp-v1** — IAB CoMP V1 metadata (Package, Scope with folded licensing, Retrieval, per-media taxonomy) as optional ext fields
...
- **`Restriction`** — constrains one axis: `FUNCTION` (what), `GEOGRAPHY` (where), `USER_TYPE` (who). Tokens are proto-native (`(fora.v1.vocab_enum)` on the `RestrictionKind` values). restrictions are **binding by default** (`advisory=true` downgrades an unverifiable restriction to non-blocking).
- **`Quota`** — usage cap that gates term validity, not billing. Metrics are proto-native (`(fora.v1.vocab)` on `Quota.metric`): `accesses`, `tokens`, `display-words`, `impressions`, `units-manufactured`, and more.
...
Protocol Buffers. Package `fora.v1`. Dual transport via Connect (HTTP/JSON + binary protobuf from the same handler).
