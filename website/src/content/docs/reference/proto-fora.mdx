...
One complete access arrangement for a resource. Lives at both ingestion (`ResourceEntry.terms`, many per resource) and emission (`Offer.terms`, exactly one per offer).
...
**Reading a restriction**: a value is in-scope when it matches at least one `permitted[]` token AND none of the `prohibited[]` tokens. Empty `permitted[]` = any value permitted on this axis. Restrictions ride on the offer, which carries exactly one term; the agent chooses among the offers a resource yields, one per entitled term (the Exchange does not pre-filter terms against requester attributes).
