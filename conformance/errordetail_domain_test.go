...
// surface, e.g. fora.v1.ExchangeService", and ADR-019 hedges the same way — an example,
...
//     carrying fora.v1.ExchangeService came from that service.
//   - `<package>.<BareNoun>` names a TIER that is not a service and appears in no
//     descriptor — fora.v1.Edge for a delivery edge, fora.v1.Client for a refusal the
...
// this guard was written for — a corpus fixture naming fora.v1.RegistrationService, a
// service the contract has never defined, sitting beside two sibling vectors that use
// fora.v1.ExchangeService for the same reason block.
//
...
// The tiers that are not services live there too: fora.v1.Edge is not a proto symbol, but
...
func TestErrorDetailDomain_DetectorAnswersForBothShapes(t *testing.T) {
	...
	for _, name := range []string{"fora.v1.ExchangeService", "fora.v1.CatalogService"} {
		if !services[name] {
			t.Errorf("%s is not among the contract's services, so the guard's own "+
				"positive case is wrong", name)
		}
	}
	for _, name := range []string{"fora.v1.Edge", "fora.v1.Client", "fora.v1.RegistrationService"} {
		if services[name] {
			t.Errorf("%s is a contract service, which this guard assumes it is not", name)
		}
	}
	for _, in := range []string{"fora.v1.Edge", "fora.admin.v1.AdminService"} {
		if !inAContractPackage(in) {
			t.Errorf("%q was read as outside every contract package", in)
		}
	}
	for _, out := range []string{"", "Edge", "google.rpc.ErrorInfo", "fora.v1.a.b"} {
